...
TP<CL T, CL = void> IC BL is_tuple_like = false;
TP<CL T> IC BL is_tuple_like<T, void_t<decltype(tuple_size<T> {})>> = true;
...
TP<CL T> using iter_rvalue_reference_t = iter_reference_t<T>; // [[todo]] = decltype(_RG iter_move(DCLV(T&)));
//[default.sentinel]
ST default_sentinel_t {}; IC default_sentinel_t default_sentinel {};
//[unreachable.sentinel]
ST unreachable_sentinel_t { //[[todo] : I is weakly_incrementable]
TP<CL I> BL FCOP ==(I, unreachable_sentinel_t) { return false; }
TP<CL I> BL FCOP !=(I, unreachable_sentinel_t) { return true; }
TP<CL I> BL FCOP ==(unreachable_sentinel_t, I) { return false; }
TP<CL I> BL FCOP !=(unreachable_sentinel_t, I) { return true; } 
};
IC unreachable_sentinel_t unreachable_sentinel {};
//[iter.concept]
TP<CL,CL=void>IC BL has_iter_tag = false;
TP<CL T>IC BL has_iter_tag<T, void_t<typename iterator_traits<T>::iterator_category>> = true;
TP<CL,CL=void>IC BL has_iter_concept = false;
TP<CL T>IC BL has_iter_concept<T, void_t<typename T::iterator_concept>> = true;
...
TP<CL T> using iterator_t = decltype(_RG begin(DCLV(T&)));
TP<CL T> using sentinel_t = decltype(_RG end(DCLV(T&)));
TP<CL R> using range_size_t = decltype(_RG size(DCLV(R&)));
...
RetReq(same_as, add_pointer_t<range_reference_t<T>>) (_RG data(t))
);
...
LazyT(D,ReqExpr(_RG empty(DCLV(t&)))) explicit COP bool() RET(!_RG empty(derived()) )
LazyT(D,ReqExpr(_RG empty(DCLV(t&)))) explicit COP bool() const RET(!_RG empty(derived()) )
LazyT(D,ReqExpr(range<D>&&contiguous_iterator<iterator_t<t>>)) CEXP auto data()
->decltype(&*_RG begin(DCLV(t&)))RET(&*begin(derived()))
LazyT(D,ReqExpr(range<const D>&&contiguous_iterator<iterator_t<const t>>)) CEXP auto data()
const->decltype(&*_RG begin(DCLV(const t&))) RET(&*begin(derived()))
LazyT(D,forward_range<t> && sized_sentinel_for<sentinel_t<t>, iterator_t<t>>)
CEXP auto size() RET(_RG end(derived()) - _RG begin(derived()))
...
static IC BL fw=derived_from<Tg, forward_iterator_tag>;
static IC BL bd=derived_from<Tg, bidirectional_iterator_tag>;
static IC BL ra=derived_from<Tg, random_access_iterator_tag>;
using R=I&;using CR=const I&;
R CEXP It()RET(R(*this)) CR CEXP It()const RET(CR(*this))
#define RT(...) { __VA_ARGS__; return R(*this); }
public:using pointer=void;
...
CEXP subrange(R&&r):subrange(r,_RG size(r)){}
TpReq(CL R)(borrowed_range<R>&&convertible_to_non_slicing<iterator_t<R>,I>&&convertible_to<sentinel_t<R>,S> && !StoreSize)
subrange(R&&r):subrange(_RG begin(r),_RG end(r)) {}
...
CEXP subrange(R&&r,sz_t n):subrange(_RG begin(r), _RG end(r), n) {}
...
CEXP auto begin() const RET(_RG begin(*r))
CEXP auto end() const RET(_RG end(*r))
LazyT(R, ReqExpr(_RG empty(*DCLV(t)))) CEXP BL empty() const RET(_RG empty(*r))
LazyT(R, sized_range<R>)CEXP auto size() const RET(_RG size(*r))
LazyT(R, contiguous_range<R>)CEXP auto data() const RET(_RG data(*r))
};
...
TP<bool> ST S;
TP<BL C>using B=maybe_const<C, V>;
TP<BL C>using D=range_difference_t<B<C>>;
TP<BL C>using Tg=iter_concept<B<C>>;
TP<CL I, BL C>using Fa=IB<I,Tg<C>,D<C>>;
TP<BL C>CL I : public Fa<I<C>,C> {
friend Fa<I,C>;
TP<bool> friend ST S;
TP<bool> friend ST I;
...
IB i_; P* p_;
CEXP void inc(){++i_;}
LazyT(B,true)CEXP void dec(){--i_;}
LazyT(B,true)void adv(D<C> n){i_+=n;}
LazyT(B,equality_comparable<IB>) BL FC eq Crefp(I)RET(i.i_==j.i_)
LazyT(B,true)BL FC lt Crefp(I)RET(i.i_<j.i_)
LazyT(B,true)D<C>FC dif Crefp(I) RET(i.i_ - j.i_)
public:
...
using pointer=void;
CEXP I(P& p, IB cur) : p_(&p), i_(cur) {}
LazyT(V, C && convertible_to<IV,IB>) CEXP I(I<!C> i) : i_(move(i.i_)), p_(i.p_) {}
CEXP auto base() const& RET(i_)
CEXP auto base() && RET(move(i_))
DCLT(auto) COP*() const RET( my_invoke(*p_->fun_, *i_) )
FC DCLT(auto) iter_move(const I&i) NOEXP( _RG invoke(*p_->fun_, *i_) ) 
...
NOEXP_RET(_RG iter_swap(i.i_, j.i_) )
#undef TMPR
};
TP<BL C> CL S {
TP<bool> friend ST s;
...
sentinel_t<B> s_;
public:
CEXP S(sentinel_t<B> end) : s_(move(end)){}
LazyT(V, C && convertible_to<sentinel_t<V>, sentinel_t<B>>) CEXP S(S<!C> i) : s_(move(i.s_)){}
CEXP auto base() RET(s_)
BL FCOP ==(const I<C>& i, const S& j) RET( i.base() == j.s_ )
BL FCOP ==(const S& i, const I<C>& j) RET(j==i)
BL FCOP !=(const I<C>& i, const S& j) RET(!(i==j))
BL FCOP !=(const S& i, const I<C>& j) RET(j!=i)
#define Def LazyT(B,sized_sentinel_for<sentinel_t<B>, iterator_t<B>>) range_difference_t<B> FCOP - 
Def (const I<C>& i, const S& j) RET( i.i_ - j.s_ )
Def (const S& i, const I<C>& j) RET( i.s_ - j.i_ )
#undef Def
};
V base_ = V();
copyable_box<F> fun_;
...
TP<CL VV=V>RI<> CEXP begin() { return make_reverse_iterator(_RG next(::begin(base_), ::end(base_))); }
TP<CL VV=V>RI<range<const VV>>CEXP begin() const 
{ return make_reverse_iterator(_RG next(::begin(base_), ::end(base_))); }
...
RET(find_if_fn::impl(_RG begin(r), _RG end(r), not_fn(ref(pr)), ref(p)))
} find_if_not;
//[range.take.while]
...
CEXP auto begin() RET(_RG find_if_not(v_, cref(*p_)))
CEXP auto end() RET(_RG end(v_))
...
TP<CL... R>concept zip_is_common = (sizeof...(R)==1&&(common_range<R> && ...))||(!(bidirectional_range<R>&&...)&&(common_range<R>&&...))||((random_access_range<R>&&sized_range<R>)&&...);
...
TP<BL C> ST I:IB<I<C>,Tg<C>,D<C>>{
TP<BL>friend ST S;friend IB<I<C>,Tg<C>,D<C>>;
#define TMCV(Name) tuple_or_pair<Name<MCV>...>
CEXP I(TMCV(iterator_t) i_) : i_(move(i_)) {}
using iterator_concept=Tg<C>;using iterator_category=Tg<C>;using value_type = TMCV(range_value_t);
using reference = TMCV(range_reference_t);using difference_type=D<C>;
...
void FC iter_move(const I&i)RET(ttf_(_RG iter_move,i.i_))
CurLazy((indirectly_swappable<iterator_t<MCV>>&&...))void FC iter_swap Crefp(I){tfe_(_RG iter_swap,i.i_,j.i_);}
...
CurLazy(true)D<C> FC dif Crefp(I)RET(apply([](auto... b) RET(min({(D_Ty)b...}, {},abs)),ttf_([](auto&i, auto&j)RET(i-j),i.i_,j.i_)))
#undef All_
};
...
RET(apply([](auto...b)RET(_RG min({(iter_difference_t<I<CC>>)b...},{},_RG abs)),ttf_([](auto&i, auto&j)RET(i-j),s_,i.i_)))
#undef Def
};
#undef TMCV
// TP<BL C> CL S {
// friend ST zip_view;
// #define BT tuple_or_pair<sentinel_t<MCV>...>
// BT s_;
...
// #define Def(Name) TpReq(BL CC)((Name##sentinel_for<sentinel_t<MCV>, iterator_t<maybe_const<CC, V>>> && ...)) BL FCOP
// Def() ==(const I<CC>& i, const se& j)

// Def()==(const se& i, const I<CC>& j) RET(j==i)
// Def()!=(const I<CC>& i, const se& j) RET(!(i==j))
// Def()!=(const se& i, const I<CC>& j) RET(j!=i)
// Def(sized_)-(const I<CC>& i, const se& j) 
// Def(sized_)-(const se& i, const I<CC>& j) RET(j-i)
// #undef Def
// };
#undef MCV
public:
zip_view(V... v) : v_(move(v)...) {}
CurLazy(!(simple_view<V> && ...)) CEXP I<false> begin() RET(ttf_(_RG begin, v_))
CurLazy((range<const V> && ...)) CEXP I<true> begin() const RET(ttf_(_RG begin, v_))
CurLazy(!(simple_view<V> && ...)) CEXP auto end() {
if CEXP (!zip_is_common<V...>) return S<false>(ttf_(_RG end, v_));
else if CEXP ((random_access_range<V> && ...)) return begin() + size();
else return I<false>(ttf_(_RG end, v_));
}
CurLazy((range<const V> && ...)) CEXP auto end() {
if CEXP (!zip_is_common<const V...>) return S<false>(ttf_(_RG end, v_));
else if CEXP ((random_access_range<const V> && ...)) return begin() + size();
else return I<false>(ttf_(_RG end, v_));
}
CurLazy((sized_range<V> && ...)) auto CEXP size()
RET(apply([](auto...a)RET(_RG min({(make_unsigned_t<common_type_t<DCLT(a)...>>)a...})),ttf_(_RG size, v_)))
...
BL FCOP==(const iterator& i, const iterator& j) noexcept { return i.cur == j.cur; }
BL FCOP!=(const iterator& i, const iterator& j) noexcept { return i.cur != j.cur; }
BL FCOP==(const iterator& i, default_sentinel_t) noexcept { return i.cur == i.t; }
BL FCOP==(default_sentinel_t, const iterator& i) noexcept { return i.cur == i.t; }
BL FCOP!=(const iterator& i, default_sentinel_t) noexcept { return i.cur != i.t; }
BL FCOP!=(default_sentinel_t, const iterator& i) noexcept { return i.cur != i.t; }
private:
T t, cur;
...
BL FCOP==(const iterator& i, const iterator& j) noexcept { return i.x == j.x && i.i == j.i; }
BL FCOP!=(const iterator& i, const iterator& j) noexcept { return !(i==j); }
BL FCOP==(const iterator& i, default_sentinel_t) noexcept { return i.x == 1; }
BL FCOP==(default_sentinel_t, const iterator& i) noexcept { return i.x == 1; }
BL FCOP!=(const iterator& i, default_sentinel_t) noexcept { return i.x != 1; }
BL FCOP!=(default_sentinel_t, const iterator& i) noexcept { return i.x != 1; }
private:
T x, i;
...
_RG copy(r, get_inserter<range_reference_t<R>>(c, tag<1>{}));
return c;
}
...
TP<CL R, CL T = void>
using printable_range = enable_if_t<_RG range<R> && !is_tuple_like<R> &&
!is_any_of<_RG range_value_t<R>, char, wchar_t, char16_t, char32_t>, T>;
NP print_detail {
TP<CL, CL = void> IC BL has_del_impl = false;
TP<CL T> IC BL has_del_impl<T, void_t<decltype(DCLV(T&).del)>> = true;
TP<CL T> concept has_del = has_del_impl<remove_reference_t<T>>;
TP<CL, CL = void> IC BL has_bra_impl = false;
TP<CL T> IC BL has_bra_impl<T, void_t<decltype(DCLV(T&).bra)>> = true;
...
TP<CL STRM, CL Delim>
CEXP STRM& put_delim(STRM& os, BL ok, Delim d) { if (!ok) os << d.del << " "; return os; }
// [print.declaration]
...
NP std {
TP<CL... A> CEXP void swap(const tuple<A...>& i, const tuple<A...>& j){_RG tfe_(_RG swap, i, j);}
