...
TP<CL T1,typename T2,CL...Re>ST multiple_common_reference
...
TP<CL T>using iter_rvalue_reference_t=iter_reference_t<T>; // [[todo]]=decltype(Rg iter_move(DCLV(T&)));
//[default.sentinel]
...
RetReq(same_as,add_pointer_t<range_reference_t<T>>) (Rg data(t))
);
...
public:using iterator_concept=Tg;using iterator_category=Tg;using value_type=W;using difference_type=D;using reference=W;
I()=default;CEXP explicit I(W v):v(v){}W COP*()const RET(v)
};
ST S : SB<S,I>{
...
friend Fa<I,C>;TP<bool>friend ST S;TP<bool>friend ST I;
...
IB i_;P*p_;public:using iterator_concept=Tg<C>;using iterator_category=Tg<C>;
using value_type=remove_cvref_t<invoke_result_t<F&,range_reference_t<B>>>; //maybe_const<C,F>&?[[todo]]
using difference_type=D<C>;using reference=invoke_result_t<F&,range_reference_t<B>>;
...
RET(find_if_fn::impl(Rg begin(r),Rg end(r),not_fn(ref(pr)),ref(p)))
} find_if_not;
//[range.take.while]
...
I()=default;CEXP I(TMCV(iterator_t) i_):i_(move(i_)) {}
...
if CEXP (!zip_is_common<V...>) return S<false>(ttf_(Rg end,v_));
...
if CEXP (!zip_is_common<const V...>) return S<false>(ttf_(Rg end,v_));
...
TP<CL S,TPP W,CL R,CL...Re,CL=Req(Rg range<R>),CL=void_t<typename W<R,Re...>::fmt>>void impl(S&s,W<R,Re...>,tag<3>);
TP<CL S,TPP W,CL T,CL...Re,CL=typename tuple_size<remove_cvref_t<T>>::type,CL=void_t<typename W<T,Re...>::fmt>>
void impl(S&,W<T,Re...>w,tag<2>);
TP<CL S,CL R,CL=Req(Rg range<R>)>void impl(S&s,R&&r,tag<1>);
TP<CL S,CL T,CL=typename tuple_size<remove_cvref_t<T>>::type>void impl(S&s,T&&t,tag<0>);
//op
