...
#define MOV(...) static_cast<remove_reference_t<decltype(__VA_ARGS__)>&&>(__VA_ARGS__)
...
template<class,class...>auto test_requires_fn(long)->false_type;
template<class R, class... Args,class=decltype(&R::template frequires_<Args...>)>auto test_requires_fn(int)->true_type;
template<class R, class... Args> inline constexpr bool requires_ = decltype(test_requires_fn<R, Args...>(0))::value;
template<bool Expr,class T=int> using requires_expr = enable_if_t<Expr, T>;
template<std::size_t I> struct priority_tag : priority_tag<I - 1>{}; template<>struct priority_tag<0> {};
//[remove.cvref]
template<class T>struct remove_cvref { using type = std::remove_cv_t<remove_reference_t<T>>; };
...
template<class T, class U> inline constexpr bool has_common_ref_v = exists_v<common_ref_t, T, U>;
...
template<class T, class U> inline constexpr bool has_basic_common_ref_v = exists_v<basic_common_ref_t, T, U>;
template<class T, class U> inline constexpr bool has_cond_res_v = exists_v<cond_res_t, T, U>;
...
template<class T, class U> using ternary_return_t = decay_t<decltype(false ? std::declval<T>() : std::declval<U>())>;
...
struct derived_from_concept {
    ...
    template<class Derived,class Base>static auto test(int)->enable_if_t<
        is_base_of_v<Base, Derived>&&is_convertible_v<const volatile Derived*, const volatile Base*>,true_type>;
};
template<class Derived,class Base>concept derived_from =decltype(derived_from_concept::test<Derived, Base>(0))::value;
// [concept.convertible]
struct convertible_to_concept 
{ template<class From, class To> auto frequires_(add_rvalue_reference_t<From> (&f)())->decltype(static_cast<To>(f())); };
...
template<class T, class U>auto frequires_()->
...
template<class LHS,class RHS>auto frequires_(LHS lhs, RHS&&rhs)->decltype(
...
template<class, class = void>INLINE_BOOL is_default_initializable = false;
// Thanks to Damian Jarek on Slack for this formulation
template<class T>INLINE_BOOL is_default_initializable<T, void_t<decltype(::new T)>> = true;
struct default_initializable_concept {template<class T, class = decltype(T{})>auto frequires_() -> void;};
...
move_constructible<T> && assignable_from<T&, T>> { T temp = std::move(a); a = std::move(b); b = std::move(temp); }
public: template<class T, class U> constexpr auto operator()(T&& t, U&& u) const
...
inline constexpr swap_::fn __swap;
// [concept.swappable]
struct swappable_concept { template<class T>auto frequires_(T& a, T& b) -> decltype(__swap(a, b)); };
template<class T>concept swappable = requires_<swappable_concept, T>;
struct swappable_with_concept {
template<class T, class U>auto frequires_(T&& t, U&& u) -> decltype(
    __swap(FWD(t), FWD(t)), __swap(FWD(u), FWD(u)), __swap(FWD(t), FWD(u)), __swap(FWD(u), FWD(t)) );
template<class, class>static auto test(long) -> false_type;
template<class T, class U>static auto test(int) -> enable_if_t<
...
{ template<class T>auto frequires_(T&& t) ->requires_expr<boolean_testable_impl<decltype(!FWD(t))>>; };
...
template<class T, class U> auto frequires_(const remove_reference_t<T>& t, const remove_reference_t<U>& u)->decltype(
...
struct fn { private:
    ...
    noexcept(noexcept((forward<Derived>(ref).*pmf)(forward<Args>(args)...)))
        -> enable_if_t<is_function_v<T> && is_base_of_v<Base, decay_t<Derived>>,
    ...
    ->enable_if_t<is_function_v<T> && is_reference_wrapper_v<decay_t<RefWrap>>,
    ...
    -> enable_if_t<is_function_v<T> && !is_reference_wrapper_v<decay_t<Pointer>> && !is_base_of_v<Base, decay_t<Pointer>>,
    ...
    impl(T Base::*pmd, Derived&& ref) noexcept(noexcept(forward<Derived>(ref).*pmd))
        -> enable_if_t< !is_function_v<T> && is_base_of_v<Base, decay_t<Derived>>,decltype(forward<Derived>(ref).*pmd)>
    { return forward<Derived>(ref).*pmd; }
    template<class Base, class T, class RefWrap> static constexpr auto 
    impl(T Base::*pmd, RefWrap&& ref) noexcept(noexcept(ref.get().*pmd))
    -> enable_if_t<!is_function_v<T> && is_reference_wrapper_v<decay_t<RefWrap>>, decltype(ref.get().*pmd)> 
    { return ref.get().*pmd; }
    template<class Base, class T, class Pointer> static constexpr auto
    impl(T Base::*pmd, Pointer&& ptr) noexcept(noexcept((*forward<Pointer>(ptr)).*pmd))
        -> enable_if_t< !is_function_v<T> && !is_reference_wrapper_v<decay_t<Pointer>> &&
            !is_base_of_v<Base, decay_t<Pointer>>, decltype((*forward<Pointer>(ptr)).*pmd)>
    { return (*forward<Pointer>(ptr)).*pmd; }
    template<class F, class... Args>static constexpr auto impl(F&& f, Args&&... args) 
    noexcept(noexcept(forward<F>(f)(forward<Args>(args)...)))
    -> enable_if_t<is_member_pointer_v<decay_t<F>>, decltype(forward<F>(f)(forward<Args>(args)...))>
    { return forward<F>(f)(forward<Args>(args)...); }
public:
    template<class F, class... Args> constexpr auto operator()(F&& f, Args&&... args) const 
    noexcept(noexcept(fn::impl(forward<F>(f), forward<Args>(args)...)))
    -> decltype(fn::impl(forward<F>(f), forward<Args>(args)...))
    { return fn::impl(forward<F>(f), forward<Args>(args)...); }
};
} // namespace invoke_
inline constexpr invoke_::fn __invoke;
template<class Void, class, class...> struct invoke_result_helper {};
template<class F, class... Args>
struct invoke_result_helper<void_t<decltype(__invoke(declval<F>(), declval<Args>()...))>,F, Args...> {
...
template<class T>static auto test(long)->false_type; template<class T>static auto test(int)->enable_if_t<
is_object_v<T> && move_constructible<T> &&assignable_from<T&, T> && swappable<T>,true_type>;
};
template<class T>concept movable = decltype(movable_concept::test<T>(0))::value;
// [concept.copyable]
struct copyable_concept {       template<class>static auto test(long) -> false_type;
    template<class T>static auto test(int) -> enable_if_t< copy_constructible<T> && movable<T> && assignable_from<T&, T&> &&
        assignable_from<T&, const T&> && assignable_from<T&, const T>,true_type>;
};
template<class T>concept copyable = decltype(copyable_concept::test<T>(0))::value;
// [concept.semiregular]
...
inline constexpr struct auto_fn { 
    template<class T> decay_t<T> constexpr operator()(T&& t) const { return FWD(t); }
    template<class T> decay_t<T> friend constexpr operator%(T&& t, auto_fn) { return FWD(t); }
} Auto;
// [utility.move]
inline constexpr struct move_fn { 
    template<class T> decltype(auto) constexpr operator()(T&& t) const { return MOV(t); }
    template<class T> decltype(auto) friend constexpr operator%(T&& t, move_fn) { return MOV(t); }
} Move;
// [utility.Ycomb]
...
template<class T, class U = T> using bit_require = enable_if_t<is_integral_v<T>,U>;// && is_unsigned_v<T>, U>;
...
struct equal_to {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const
        -> enable_if_t<equality_comparable_with<T, U>, bool> { return FWD(t) == FWD(u); }
    using is_transparent = true_type;
};
struct not_equal_to {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const
        -> enable_if_t<equality_comparable_with<T, U>, bool> { return !ranges::equal_to{}(FWD(t), FWD(u)); }
    using is_transparent = true_type;
};
struct less {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const -> enable_if_t<totally_ordered_with<T, U>, bool>
    { return std::less<>{}(FWD(t), FWD(u)); }
    using is_transparent = true_type;
};
struct greater {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const -> enable_if_t<totally_ordered_with<T, U>, bool>
...
struct greater_equal {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const
    -> enable_if_t<totally_ordered_with<T, U>, bool> { return !ranges::less{}(FWD(t), FWD(u)); }
    using is_transparent = true_type;
};
struct less_equal {
    template<class T, class U> constexpr auto operator()(T&& t, U&& u) const -> enable_if_t<totally_ordered_with<T, U>, bool>
...
//[iter.concept]
template<class,class=void>INLINE_BOOL has_iter_tag = false;
template<class T>INLINE_BOOL has_iter_tag<T, void_t<typename iterator_traits<T>::iterator_category>> = true;
template<class,class=void>INLINE_BOOL has_iter_concept = false;
template<class T>INLINE_BOOL has_iter_concept<T, void_t<typename T::iterator_concept>> = true;
template<class T>constexpr auto iter_concept_impl() {
    ...
    else return typename iterator_traits<T>::iterator_category {};
}
template<class T>using iter_concept = decltype(iter_concept_impl<T>());
} // namespace ITER
namespace ranges {
template<class,class=void>INLINE_BOOL can_reference = false;
template<class I>INLINE_BOOL can_reference<I,void_t<I&>> = true;
template<class,class=void>INLINE_BOOL deref=false;
template<class I>INLINE_BOOL deref<I,enable_if_t<can_reference<decltype(*declval<I&>())>>> = true;
//[iterator.concept]
//[iterator.concept.winc]
struct winc { template<class I>auto frequires_(I i)-> decltype(
    // declval<iter_reference_t<I>>(), requires_expr<signed_integral<iter_reference_t<I>>>{},
...
struct sizesen {template<class S, class I>auto frequires_(const I& i, const S& s)->decltype(
...
template<class I>concept input_iterator=input_or_output_iterator<I>&& derived_from<iter_concept<I>,input_iterator_tag>;
...
using std::empty, std::begin, std::end, std::data, std::size;
...
template<class> INLINE_BOOL enable_borrowed_range = false;
...
//[range.refinements]
template<class R> concept common_range = range<R> && same_as<iterator_t<R>, sentinel_t<R>>;
...
struct has_oper_arrow {template<class I>auto frequires_(I i)->decltype(i.operator->());};
...
public:
    ...
    constexpr subrange(II i, S s) : i(MOV(i)), s{s} {}
    template<class II, requires_expr<convertible_to_non_slicing<II, I> && K == subrange_kind::sized> =0 > 
    constexpr subrange(II i, S s, size_type n) : i(MOV(i)), s{s} ,sz {n} {}
    template<class R, requires_expr<borrowed_range<R>&&convertible_to_non_slicing<iterator_t<R>,I>&&
    convertible_to<sentinel_t<R>,S> && StoreSize> =0> constexpr subrange(R&& r) : subrange(r, ranges::size(r)) {}
    template<class R, requires_expr<borrowed_range<R>&&convertible_to_non_slicing<iterator_t<R>,I>&&
    convertible_to<sentinel_t<R>,S> && !StoreSize> =0> subrange(R&& r) :subrange(ranges::begin(r), ranges::end(r)) {}
    template<class R, requires_expr<borrowed_range<R>&&convertible_to_non_slicing<iterator_t<R>,I>&&
    convertible_to<sentinel_t<R>,S> && K==subrange_kind::sized> =0> 
    constexpr subrange(R&& r, size_type n) :subrange(ranges::begin(r), ranges::end(r), n) {}
    template<class II=I>constexpr auto begin()->enable_if_t<copyable<II>, I> const { return i; }
    template<class II=I>constexpr auto begin()->enable_if_t<!copyable<II>, I> { return MOV(i); }
...
struct ref_req { static void FUN(R&); static void FUN(R&&)=delete;
    template<class T>auto frequires_()->decltype(FUN(declval<T>()));
};
    R* r;
public:
    ...
    constexpr auto begin() const { return ranges::begin(*r); }
    constexpr auto end() const { return ranges::end(*r); } 
    template<class RR=R,decltype(ranges::empty(*declval<RR>()),0) = 0> constexpr bool empty() const { return ranges::empty(*r); }
    template<class RR=R,requires_expr<sized_range<R>> =0>constexpr auto size() const { return ranges::size(*r); }
...
public:
    template<class T> constexpr auto operator()(T&& t) const noexcept(noexcept(impl(FWD(t), priority_tag<2>{})))
        -> decltype(impl(FWD(t), priority_tag<2>{})) { return impl(FWD(t), priority_tag<2>{}); }
    template<class T> constexpr friend auto operator|(T&& t,all_fn f) noexcept(noexcept(f(FWD(t))))
...
public:
    constexpr explicit reverse_view(V base) : base_((base)) {} // [todo move]
    constexpr V base() const& { return base_; }
    template<class VV=V>RI<> constexpr begin() { return make_reverse_iterator(ranges::next(ranges::begin(base_), ranges::end(base_))); }
    template<class VV=V>RI<range<const VV>>constexpr begin() const 
    { return make_reverse_iterator(ranges::next(ranges::begin(base_), ranges::end(base_))); }
    template<class VV=V>RI<>constexpr end() { return make_reverse_iterator(ranges::begin(base_)); }
    template<class VV=V>RI<range<const VV>>constexpr end() const { return make_reverse_iterator(ranges::begin(base_)); }
    template<class VV=V>RS<VV> constexpr size() { return ranges::size(base_); }
    template<class VV=V>RS<const VV> constexpr size() const { return ranges::size(base_); }
private:
    V base_;
};
...
namespace views {
    struct reverse_view_fn {
        template<class R> constexpr auto operator()(R&& r) const { return reverse_view { FWD(r) }; }
        template<class R> friend constexpr auto operator|(R&&r, reverse_view_fn f) { return f(FWD(r)); } 
    };
    inline constexpr reverse_view_fn reverse;
}
// [iota.view]
...
struct zip_fn {
    ...
    struct Range {
        ...
        struct iterator {
            ...
            constexpr iterator& operator--() { --r; --s; return *this; }
            constexpr iterator operator--(int) { auto r = *this; --*this; return r; }
            friend constexpr bool operator!=(const iterator& i, const iterator& j) { return i.r != j.r && i.s != j.s; }
            friend constexpr bool operator==(const iterator& i, const iterator& j) { return i.r == j.r && i.s == j.s; }
            friend constexpr bool operator!=(const iterator& i, const sentinel& j) { return i.r != j.r && i.s != j.s; }
            friend constexpr bool operator==(const iterator& i, const sentinel& j) { return i.r == j.r && i.s == j.s; }
            friend constexpr bool operator!=(const sentinel& i, const iterator& j) { return i.r != j.r && i.s != j.s; }
            friend constexpr bool operator==(const sentinel& i, const iterator& j) { return i.r == j.r && i.s == j.s; }
        };
        struct sentinel {
...
template<class R, class S> decltype(auto) operator()(R&& r, S&& s) const { return Range<R, S> { (R&&)r, (S&&)s };}
};
...
template<auto M = long(1e9 + 7)> struct B {
    ...
    template<class I, class = Q<I>> explicit constexpr operator I() const { return I(v); }
...
inline namespace simplify {
    ...
    CPO range=vw::iota; CPO zip=vw::zip; CPO enu=vw::enumerate; CPO subset=vw::subset;
    CPO rev=vw::reverse; CPO all = vw::all;
} // namespace simplify
